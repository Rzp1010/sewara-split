-- Migration 2026-09-17: Fitur Printilan
-- Tambah kolom printilan (jsonb) ke transactions.
-- Update RPC rpc_save_transaction untuk menulis kolom baru.
-- Jalankan manual di SQL Editor DB sewara (obhvrzholszhjnpvmnna).
BEGIN;

-- 1) Tambah kolom baru
ALTER TABLE transactions ADD COLUMN IF NOT EXISTS printilan jsonb DEFAULT '[]'::jsonb;

-- 2) Recreate RPC — tambah printilan di UPDATE dan INSERT
...
COMMENT ON FUNCTION public.rpc_save_transaction(jsonb,jsonb,jsonb,boolean,boolean) IS 'Atomic tenant-scoped save; +dp_hangus, +dp_hangus_aturan (2026-09-16); +printilan (2026-09-17).';

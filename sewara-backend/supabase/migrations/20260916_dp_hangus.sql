-- Migration 2026-09-16: Fitur DP Hangus
-- Tambah kolom dp_hangus & dp_hangus_aturan ke transactions.
-- Update RPC rpc_save_transaction untuk menulis kolom baru.
-- Jalankan manual di SQL Editor DB sewara (obhvrzholszhjnpvmnna).
BEGIN;

-- 1) Tambah kolom baru
ALTER TABLE transactions ADD COLUMN IF NOT EXISTS dp_hangus numeric DEFAULT 0;
ALTER TABLE transactions ADD COLUMN IF NOT EXISTS dp_hangus_aturan text;

-- 2) Recreate RPC — tambah dp_hangus & dp_hangus_aturan di UPDATE dan INSERT
...
DECLARE
 ...
 v_invoice text;
 v_dp_hangus numeric; -- [NEW] DP Hangus
BEGIN
 ...
 BEGIN
  ...
  v_dp_hangus=COALESCE(NULLIF(p_transaction->>'dp_hangus','')::numeric,0); -- [NEW]
...
COMMENT ON FUNCTION public.rpc_save_transaction(jsonb,jsonb,jsonb,boolean,boolean) IS 'Atomic tenant-scoped save; +dp_hangus, +dp_hangus_aturan (fix 2026-09-16).';

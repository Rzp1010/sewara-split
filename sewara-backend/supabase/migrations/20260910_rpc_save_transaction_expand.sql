-- Draft migration: jalankan manual di SQL Editor DB sewara (obhvrzholszhjnpvmnna).
-- VERIFIED live DB 2026-09-10: riwayatDilayani (jsonb), dilayani_oleh (text), status (enum), diskon (jsonb), id_transaksi (text) ADA.
-- emailPenyewa TIDAK ada di tabel transactions — diabaikan.
-- transactions.id: is_identity=NO, column_default=0 → DB TIDAK auto-generate id. Client wajib kirim id (Date.now()).
-- Perubahan vs phase2c2: (1) insert menerima id client (upsert-by-id), (2) whitelist + riwayatDilayani/dilayani_oleh.
...
BEGIN
 ...
 IF v_id IS NULL THEN RAISE EXCEPTION 'id is required (client-generated, e.g. Date.now()) — DB has no id generator' USING ERRCODE='22023'; END IF;
  ...
  ELSE
    ...
    INSERT INTO transactions(id,id_transaksi,penyewa,hp_penyewa,alamat_penyewa,jaminan_sewa,waktu_ambil_rencana,waktu_kembali_rencana,waktu_ambil_aktual,waktu_kembali_aktual,status,items,denda,biaya,durasi_teks,total_akhir,pembayaran,diskon,"riwayatDilayani",dilayani_oleh,member_id,user_id)
    VALUES(v_id,p_transaction->>'id_transaksi',p_transaction->>'penyewa',p_transaction->>'hp_penyewa',p_transaction->>'alamat_penyewa',p_transaction->>'jaminan_sewa',v_ta,v_tk,v_aa,v_ak,v_status,CASE WHEN jsonb_typeof(p_transaction->'items')='array' THEN p_transaction->'items' ELSE '[]'::jsonb END,v_denda,v_biaya,p_transaction->>'durasi_teks',v_total,CASE WHEN jsonb_typeof(p_transaction->'pembayaran')='object' THEN p_transaction->'pembayaran' ELSE '{}'::jsonb END,CASE WHEN jsonb_typeof(p_transaction->'diskon')='object' THEN p_transaction->'diskon' ELSE '{}'::jsonb END,CASE WHEN jsonb_typeof(p_transaction->'riwayatDilayani')='array' THEN p_transaction->'riwayatDilayani' ELSE '[]'::jsonb END,p_transaction->>'dilayani_oleh',v_member,v_owner) RETURNING * INTO v_parent;
  END IF;
